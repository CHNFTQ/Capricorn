...
# --------------------------------------------------------
# The script to train Capricorn and other diffsuion-based experiments under 40*40 setting.
# --------------------------------------------------------
...
parser.add_argument('--valid-per-step-num', type=int, default = 3000, help='Run validation for every this number of train steps')
...
parser.add_argument('--method-name', type=str, default='Capricorn')
...
parser.add_argument('--weights-file', type=str, default = 'weights/Multi_channel_weights_with_difficulty_modifier.json')
...
parser.add_argument('--diffusion-steps', type=int, default = 5, help='When validate, run how many steps of diffusion.')
...
train_loss = []
valid_loss = []
mse_scores = [[] for c in range(args.num_channels)]
...
for epoch in range(1, args.num_epochs + 1):
    ...
    for data, target, _ in train_bar:
        ...
        if step % args.valid_per_step_num == 0:
            train_loss.append(train_result['loss'] / train_result['nsamples'])
            logging.info(("Step: %d, \t Train, \t Loss: %0.8f" % (step, train_loss[-1])))
            ...
            with torch.no_grad():
                ...
                for data, target, _ in valid_bar:
                    batch_size = data.size(0)
                    valid_result['nsamples'] += batch_size

                    loss = trainer(target, cond_images = data, unet_number = args.train_unet)
                    valid_result['loss'] += loss * batch_size

                    valid_bar.set_description(
                        desc=f"[{epoch}/{args.num_epochs}, {step}] Valid Loss: {valid_result['loss'] / valid_result['nsamples']:.4f}")
                    
            valid_loss.append(valid_result['loss'] / valid_result['nsamples'])
            logging.info(("Step: %d, \t Valid, \t Loss: %0.8f" % (step, valid_loss[-1])))

            now_loss = valid_loss[-1]

            if now_loss < best_loss:
                best_loss = now_loss
                best_ckpt_file = 'best_loss.pt'
                print(f'Now, Best loss is {best_loss:.6f}')
                logging.info(f'Now, Best loss is {best_loss:.6f}, Saving to {best_ckpt_file}')

                trainer.save(os.path.join(out_dir, best_ckpt_file))
            ...
            for c in args.output_channels:
                mse_scores[c].append(valid_sample_result['mse'][c] / valid_sample_result['nsamples'])
            
            logging.info(f"Step: {step}, \t Valid, \t mse: {['%0.4e' % mse_scores[c][-1] for c in args.output_channels]}")


            for c in args.output_channels:
                now_mse = mse_scores[c][-1]
                if now_mse < best_mse[c]:
                    best_mse[c] = now_mse
                    best_ckpt_file = f'best_mse_{c}.pt'
                    print(f'Now, Best mse {c} is {best_mse[c]:.4e}')
                    logging.info(f'Now, Best mse {c} is {best_mse[c]:.4e}, Saving to {best_ckpt_file}')
            ...
            ax1.set_ylabel("loss")
            ax1.set_xlabel("per valid steps")
            plt.legend()
                        
            ax2 = fig.add_subplot(122)
            for c in args.output_channels:
                ax2.plot(x, mse_scores[c], label=f'valid mse {c}')

            ax2.set_ylabel("mse")
            ax2.set_xlabel("per valid steps")
            plt.legend()
            
            plt.savefig(os.path.join(out_dir, "loss_and_mse.pdf"))
            plt.close()
