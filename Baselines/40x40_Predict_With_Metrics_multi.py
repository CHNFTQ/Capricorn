...
import torch
import Models.HiCARN_1_multi as HiCARN_1_multi
import Models.HiCARN_2 as HiCARN_2_multi
...
def hicarn_predictor(model, hicarn_loader, ckpt_file, device, data_file, input_channel_num=1, output_channel_num=1):
    deepmodel = model.Generator(num_channels=64, input_channels=input_channel_num).to(device)
    ...
    with torch.no_grad():
        for batch in tqdm(hicarn_loader, desc='HiCARN Predicting: '):
            ...
            result_data.append(out.to('cpu').numpy()[:,:1,:,:])
            result_inds.append(inds.numpy())
            target_data.append(hr.to('cpu').numpy()[:,:1,:,:])
...
if __name__ == '__main__':
    args = data_predict_parsers().parse_args(sys.argv[1:])
    input_channel_num = args.input_channels
    output_channel_num = args.output_channels
    ...
    if model == "HiCARN_1_multi":
        model = HiCARN_1_multi

    if model == "HiCARN_2_multi":
        model = HiCARN_2_multi

    hicarn_hics, target_hics = hicarn_predictor(model, hicarn_loader, ckpt_file, device, hicarn_data, input_channel_num, output_channel_num)
