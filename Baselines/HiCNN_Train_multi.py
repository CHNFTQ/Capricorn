...
if args['model'] == 'HiCNN':
    netG = Generator(input_channels=input_channel_num, out_channels=out_channel_num).to(device)
else:
    raise NotImplementedError
# loss function
if args['model'] == 'HiCNN':
...
for epoch in range(1, num_epochs + 1):
    ...
    for data, target, _ in train_bar:
        ...
        if args['model'] == 'HiCNN':
            ...
            for i in range(out_channel_num):
                g_loss += criterionG(fake_img[:,i:i+1,:,:], real_img[:,i:i+1,:,:]) * weight[i]
        else:
    ...
    with torch.no_grad():
        for val_lr, val_hr, inds in valid_bar:
            ...
            if args['model'] == 'HiCNN' or args['model'] == 'HiCPlus':
