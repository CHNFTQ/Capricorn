...
def hicarn_predictor(model, hicarn_loader, ckpt_file, device, data_file, input_channel_num=1, output_channel_num=1):
    deepmodel = model.Generator(input_channels=input_channel_num, out_channels=output_channel_num).to(device)
...
if __name__ == '__main__':
    ...
    if model == "HiCSR_multi":
        model = HiCSR

    if model == "HiCNN_multi":
        model = HiCNN

    indices, compacts, sizes = data_info(hicarn_data)
    hicarn_hics, target_hics = hicarn_predictor(model, hicarn_loader, ckpt_file, device, hicarn_data, input_channel_num, output_channel_num = output_channel_num)
